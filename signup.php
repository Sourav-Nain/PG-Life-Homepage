...
$password="";
$database="pglife";
$conn=mysqli_connect("$server","$username","$password","$database"); $message =
""; if(!$conn){ die("connection failed: ".mysqli_connect_error()); }
if(isset($_POST['name'])){ $name = $_POST['name']; $phone=$_POST['phone'];
$email=$_POST['email']; $password=$_POST['password'];
$college=$_POST['college']; $gender = $_POST['gender']; $sql = "INSERT INTO
users(name, phone, email, password, college, gender)
VALUES('$name','$phone','$email','$password','$college','$gender')"; $result =
mysqli_query($conn, $sql); if($result){ $message = "Account Created
Successfully"; } else{ $message = "Error: " . mysqli_error($conn); } }
mysqli_close($conn); ?>
<!doctype html>
<html lang="en">
  <head>
  ...
  <body>
    <?php if($message != ""){ ?>
    <div
      class="w-[40%] mx-auto mt-4 p-3 bg-green-100 text-green-700 border border-green-500 rounded text-center"
    >
      <?php echo $message; ?>
    </div>
    <?php } ?>
